...
class ProxyMiddleware(object):
    ...
    def process_request(self, request, spider):
        ...
        proxy_info = self.proxy.get_random_proxies()
        ip = proxy_info['ip']
        port = proxy_info['port']
        # request.headers.setdefault('User-Agent', random.choice(USER_AGENTS))
        request.meta['proxy'] = "http://" + ip + ":" + port  # "121.33.226.167:3128"
        # pass
...
class RetryMiddleware(object):
    def __init__(self):
        # 需要重试的异常
        self.EXCEPTIONS_TO_RETRY = (TimeoutError, DNSLookupError,
                                    ConnectionRefusedError, ConnectionDone, ConnectError,
    ...
    def change_user_agent(self, request):
        request.headers.setdefault('User-Agent', random.choice(USER_AGENTS))
        return request
